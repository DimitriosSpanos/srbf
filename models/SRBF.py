...
class SRBF(nn.Module):
    ...
    def __init__(
        ...
        #self.feature_extractor.final_fc = torch.nn.Identity()
    ...
    def forward(self, x):
        if not self.post_subclass:
            ...
            selected_values0 = torch.masked_select(sub_y_pred, self.mask == 0).view(-1,int(len(self.binary_map) - torch.sum(self.mask)))
    ...
    def criterion(self, x, y, s_y):
        ...
        self.update_model2(np.array(centers), 0)
    ...
    def update_model(self, cluster_centers, class_id):
        ...
        #print(f"SRBF Centers shape: {self.m1.size()} - W_s's shape: {self.W_s.size()} - N1's shape: {self.N1.size()}")
    ...
    def update_rbf_layers(self, class_id, cluster_centers):
        ...
        self.m1 = torch.cat([self.m1[:, :class_id], torch.from_numpy(new_centers).to(device=training_device), self.m1[:, class_id + 1:]], 1)

        # W expanded from (model_output_size, centroid_size) to (centroid_size, num_subclasses, model_output_size)
        self.W_s = nn.Parameter(self.W1_part1.transpose(0,1).unsqueeze(1).repeat(1,self.m1.size(1),1).to(training_device))

        N1 = self.N1
        for _ in range(cluster_centers.shape[0] - 1):
            N1 = torch.cat((N1[:class_id], N1[class_id].unsqueeze(0), N1[class_id:]))
        self.register_buffer("N1", N1)

    def update_model2(self, cluster_centers, class_id):

        self.update_rbf_layers2(class_id, cluster_centers)
        #print(f"SRBF Centers shape: {self.m1.size()} - W_s's shape: {self.W_s.size()} - N1's shape: {self.N1.size()}")

    def update_rbf_layers2(self, class_id, cluster_centers):
        # convert centers of size e.g. (512,10) to (512,11) by duplicating "class id"-th center and
        # moving the centers towards the cluster centers of the features
        global training_device
        new_centers = np.zeros((cluster_centers.shape[1], cluster_centers.shape[0]))
